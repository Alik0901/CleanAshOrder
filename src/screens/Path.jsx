/*  src/screens/Path.jsx – preloading + on‑load timing  _____________________ */
...
/* ---------- styles (unchanged except комментарий) --------------------- */
const S = {
...
const styleTag = (
  <style>{`
    @keyframes fly {
      0%   { opacity: 0; transform: translate(-50%,-50%) scale(.3);}   /* fade‑in */
      15%  { opacity: 1; transform: translate(-50%,-50%) scale(1);}    /* full */
      65%  { opacity: 1; transform: translate(-50%,-50%) scale(1);}    /* hold */
      100% { opacity: 0; transform: translate(-50%,280%)  scale(.3);}  /* fly away */
    }
  `}</style>
);
...
export default function Path() {
  ...
  /* pre‑load all fragment images once --------------------------------- */
  useEffect(() => {
    Object.values(FRAG_IMG).forEach(file => {
      const img = new Image();
      img.src = `/fragments/${file}`;  // caching in browser
    });
  }, []);
  ...
  /* ui ---------------------------------------------------------------- */
  const [frag,       setFrag      ] = useState('');   // current fragment src
  const [fragLoaded, setFragLoaded] = useState(false); // true after img onLoad
    ...
    try {
      ...
      if (j.paid) {
        ...
        if (j.cursed) {
          ...
          setFrag(url);          // mount img (onLoad will trigger timing)
          setFragLoaded(false); // reset

          setMsg(`🔥 Fragment #${j.newFragment} received!`);
        }
      }
    } catch (e) { setMsg(e.message); setBusy(false); setWait(false); }
  };

  /* when image finished loading, keep it visible for full animation time */
  useEffect(() => {
    if (fragLoaded) {
      const tm = setTimeout(() => {
        setFrag('');
        setFragLoaded(false);
      }, 2300); // equals animation duration
      ...
      <div style={S.page}>
        <div style={S.card}>
          ...
          <button style={{ ...S.btn, ...S.prim, opacity: disabled ? 0.6 : 1 }}
                  disabled={disabled} onClick={() => setModal(true)}>
            {mainTxt}
          </button>
