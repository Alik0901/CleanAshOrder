          ...
          <div key={i} style={S.row}>
            {r.map(id=>(
              <div key={id} style={S.slot}>
                {frags.includes(id)&&(
                  <img
                    src={`${BACKEND}/api/fragments/image/fragment_${id}_${SLUG[id-1]}.webp?token=${localStorage.getItem('token')}`}
                    style={S.img}
                    onClick={()=>setZoomSrc(
                      `${BACKEND}/api/fragments/image/fragment_${id}_${SLUG[id-1]}.webp?token=${localStorage.getItem('token')}`
                    )}
                  />
                )}
              </div>
