...
/* Стили (убраны лишние отступы, всё центрируется с inset:0) */
const S = {
  page:{
  ...
  wrap:{
    ...
    background:'#0007', backdropFilter:'blur(4px)',
    display:'flex', justifyContent:'center', alignItems:'center',
    zIndex:40
  },
  box:{
    background:'#222', padding:24,
...
export default function Profile() {
  ...
  /* защищённые картинки */
  const [images,  setImages ] = useState({}); // { id: blobURL }
  ...
  /* подгрузка изображений фрагментов как Blob */
  useEffect(() => {
    const token = localStorage.getItem('token');
    frags.forEach(id => {
      if (images[id]) return;
      const filename = `fragment_${id}_${SLUG[id-1]}.webp`;
      fetch(`${BACKEND}/api/fragments/image/${filename}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(r => {
    ...
    <div style={S.page}>
      <div style={S.card}>
          ...
          <div key={i} style={S.row}>
            {r.map(id=>(
              <div key={id} style={S.slot}>
                {frags.includes(id) && images[id] && (
                  <img
                    src={images[id]}
                    style={S.img}
                    onClick={()=>setZoomSrc(images[id])}
                  />
                )}
              </div>
            ))}
        ...
        <div style={S.refBox}>
          ...
          <p style={S.progress}>Invited {Math.min(invCnt,3)}/3</p>
          {!reward && invCnt >= 3 && (
