/* src/screens/Profile.jsx – v3.7 (c защитой фрагментов) */
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if ([401,403,404].includes(r.status)) throw new Error();
  ...
  const claim = async () => {
    ...
    try {
      ...
      setReward(true);
      if (fragment!=null) setFrags(f=>[...f,fragment]);
              ...
              <div key={id} style={S.slot}>
                {frags.includes(id)&&(
                  <img
                    ...
                    style={S.img}
                    onClick={()=>setZoomSrc(
                      `${BACKEND}/api/fragments/image/fragment_${id}_${SLUG[id-1]}.webp`
                    )}
                  />
                )}
              </div>
      ...
      {askDel&&(
        <div style={S.wrap} onClick={()=>!busyDel&&setAskDel(false)}>
          <div style={S.box} onClick={e=>e.stopPropagation()}>
            ...
            {delErr&&<p style={{color:'#f66',fontSize:14}}>{delErr}</p>}
            <button style={S.ok}
                    disabled={busyDel}
                    onClick={deleteProfile}>
              {busyDel?'Deleting…':'Yes, delete'}
            </button>
            <button style={S.cancel}
                    disabled={busyDel}
...
const S = {
  ...
  slot:{ flex:'1 1 0',aspectRatio:'1/1',
